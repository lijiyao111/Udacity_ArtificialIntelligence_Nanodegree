...
## Do some work to understand the AIMA code:
    for a in p.actions_list:
        print(a.name,a.args)
        result_state=p.result('TT',a)
        print("result: ",result_state)
        kb = PropKB()
        fluent=FluentState([expr('Have(Cake)')],[expr('Eaten(Cake)')])
        state_map=fluent.pos+fluent.neg
        kb.tell(decode_state(result_state, fluent.pos+fluent.neg).pos_sentence())
        print(decode_state(result_state, fluent.pos+fluent.neg).pos_sentence())
        print(kb.clauses)
        new_state=FluentState([],[])
        new_state.pos.append(kb.clauses[0])
        print(encode_state(new_state, state_map))
    ...
    run_search(p, astar_search, parameter=p.h_ignore_preconditions)
    print("A-star levelsum heuristic")
    run_search(p, astar_search, parameter=p.h_pg_levelsum)