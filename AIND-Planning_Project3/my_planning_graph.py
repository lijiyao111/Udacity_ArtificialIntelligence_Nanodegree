...
class PlanningGraph():
    ...
    def create_graph(self):
        ...
        for literal in self.fs.pos:
            ...
            # PgNode_s(literal, True).show()
        for literal in self.fs.neg:
            self.s_levels[level].add(PgNode_s(literal, False))
            # PgNode_s(literal, False).show()
        ...
        # ## Print the planning graph
        # print()
        # print("in S_level {}:".format(0))
        # for s in self.s_levels[0]:
        #     s.show()

        # for i in range(level):
        #     print()
        #     print("in A_level {}:".format(i))
        #     for a in self.a_levels[i]:
        #         a.show()
        #     print()
        #     print("in S_level {}:".format(i+1))
        #     for s in self.s_levels[i+1]:
        #         s.show()
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            is_possible = True
            for p in a_node.prenodes:
                if not p in self.s_levels[level]:
                    is_possible = False
                    break

            if is_possible:
                self.a_levels[level].add(a_node)
                for s in self.s_levels[level]: # Make sure the parents and children are correctly updated
    ...
    def add_literal_level(self, level):
        ...
        s_levels_list=list()  # Make sure the parents and children are correctly updated
        for a_node in self.a_levels[level-1]:
            for a_node_eff in a_node.effnodes:
                if not a_node_eff in s_levels_list:
                    a_node.children.add(a_node_eff)
                    a_node_eff.parents.add(a_node)
                    s_levels_list.append(a_node_eff)
                else:
                    indx=s_levels_list.index(a_node_eff)
                    s_levels_list[indx].parents.add(a_node)
                    a_node.children.add(s_levels_list[indx])

        self.s_levels[level].update(s_levels_list)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a_eff in node_a1.effnodes:
            for b_eff in node_a2.effnodes:
                if self.negation_mutex(a_eff,b_eff):
                    return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a_pre in node_a1.prenodes:
            for b_eff in node_a2.effnodes:
                if self.negation_mutex(a_pre,b_eff):
                    return True
        for a_eff in node_a1.effnodes:
            for b_pre in node_a2.prenodes:
                if self.negation_mutex(a_eff,b_pre):
                    return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a_pre in node_a1.parents:
            for b_pre in node_a2.parents:
                if a_pre.is_mutex(b_pre):
                    return True
        return False
